	...
	private void initialise() {
		...
		searchField.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//int index = new BoyerMooreSearch(pattern,text).search(pattern,text);
				//int index = new BruteForceSearch(pattern,text).bruteForceSearch(pattern,text);
				if (index == -1) {
...
//		controls.add(Box.createHorizontalGlue());
//		controls.add(new JLabel("Brute Force Search"));
			...
			public boolean dispatchKeyEvent(KeyEvent ev) {
				if (ev.getID() == KeyEvent.KEY_PRESSED) {
					...
					if (shortcut >= 0) {
						...
						compressionInputFile = new File(home + "/a5_io/input_" + shortcut + ".txt");
						compressionOutputFile = new File(home + "/a5_io/output_" + shortcut + ".txt");
						String inputName = compressionInputFile == null ? "" : compressionInputFile.getName();
						String outputName = compressionOutputFile == null ? "" : compressionOutputFile.getName();
						compressionFileSelection
								.setText("<html>input: " + inputName + "<br>output: " + outputName + "</html>");
